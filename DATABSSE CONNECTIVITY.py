...
def getInfo():
    ...
    while True:
        print("\nPlease enter the symptoms you are experiencing :")
        symptoms_input = input("").strip()
        symptoms = [symptom.strip() for symptom in symptoms_input.split(",")]

        if all(symptom.isalpha() for symptom in symptoms):
            break
        else:
            print("Please enter valid symptoms (alphabetic characters only).")
